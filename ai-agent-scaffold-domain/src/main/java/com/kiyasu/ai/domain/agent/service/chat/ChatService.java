package com.kiyasu.ai.domain.agent.service.chat;
    ...
    public List<String> handleMessage(ChatCommandEntity chatCommandEntity) {
        ...
        if (null != files && !files.isEmpty()) {
            for (ChatCommandEntity.Content.File file : files) {
                parts.add(Part.fromUri(file.getFileUri(), file.getMiniType()));
            }
        }
        ...
        if (null != inlineDatas && !inlineDatas.isEmpty()) {
            for (ChatCommandEntity.Content.InlineData inlineData : inlineDatas) {
                parts.add(Part.fromBytes(inlineData.getBytes(), inlineData.getMiniType()));
            }
        }
